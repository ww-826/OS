...
`load_icode` 函数的主要功能是将一个 ELF 格式的二进制程序加载到当前进程的内存空间中。我们需要完成的是第6步：**建立用户进程的 Trapframe**，以便内核在返回用户态时，CPU 能够跳转到正确的入口地址，并拥有正确的栈指针和特权级。
...
tf->epc = elf->e_entry;
// 设置状态寄存器
// 1. 清除 SSTATUS_SPP 位：确保 sret 返回后处于 User Mode (0)
// 2. 设置 SSTATUS_SPIE 位：确保返回用户态后中断是开启的
tf->status = (sstatus & ~SSTATUS_SPP) | SSTATUS_SPIE;
```

### 2. 进程从 RUNNING 到执行第一条指令的经过

1.  **调度器选择**：内核调度器（`schedule`）决定运行该进程，将其状态设为 `PROC_RUNNING`。
2.  **上下文切换**：调用 `proc_run`，进而调用 `switch_to`，将 CPU 的上下文（寄存器 `ra`, `sp`, `s0`-`s11`）切换到该进程的内核栈上下文。
3.  **返回内核入口**：`switch_to` 返回后，CPU 跳转到 `forkret` 函数（这是在 `copy_thread` 中设置的 `ra`）。
4.  **跳转到中断出口**：`forkret` 调用 `forkrets`，传入当前进程的 `trapframe`。
5.  **恢复现场**：`forkrets` 跳转到 `__trapret`（在 `trapentry.S` 中定义）。这段汇编代码会从 `trapframe` 中恢复所有的通用寄存器（包括 `sp`，此时变为 `USTACKTOP`）和 `sstatus`、`sepc`。
6.  **特权级切换**：执行 `sret` 指令。CPU 根据 `sstatus` 的 SPP 位（已被设为 0）切换到用户态，并跳转到 `sepc` 指向的地址（即 `elf->e_entry`）。
7.  **执行指令**：CPU 开始执行用户程序的第一条指令。

## 练习2: 父进程复制自己的内存空间给子进程

### 1. 设计实现过程

`copy_range` 函数用于在 `fork` 时将父进程的内存空间（代码、数据）复制给子进程。我们需要实现具体的**物理页拷贝**和**页表映射**。

代码实现如下：
```c
// 1. 获取源页（父进程）和目标页（子进程）的内核虚拟地址
void *src_kvaddr = page2kva(page);
void *dst_kvaddr = page2kva(npage);
// 2. 复制内存内容
memcpy(dst_kvaddr, src_kvaddr, PGSIZE);
// 3. 建立映射：将填好数据的 npage 插入到子进程页表 to 中
//    线性地址为 start，权限 perm 与父进程保持一致
...
**基本概念**：
在 `fork` 时不立即复制物理内存，而是让父子进程共享同一份物理内存，并将页表项设为**只读**。只有当其中一方尝试写入时，才触发缺页异常，由内核分配新的物理页并复制数据，然后修改页表为可写。

**设计方案**：

1.  **修改 `copy_range` (在 `fork` 阶段)**：
    - 不再申请新页 (`alloc_page`) 和复制数据 (`memcpy`)。
    - 直接将父进程的物理页映射到子进程的页表中。
    - **关键点**：将父子进程该页的页表项权限都设为 **只读 (Read-Only)**，即去掉 `PTE_W` 位。
...
这些函数通常通过系统调用（System Call）从用户态陷入内核态执行。
...
**交错执行与返回**：
用户程序通过 `ecall` 陷入内核，内核处理完后通过 `sret` 返回用户态。返回值通常保存在 `a0` 寄存器中（在 `trapframe` 中修改 `gpr.a0`）。
...
# 附录：Lab2 分支任务 - QEMU 页表查询过程调试

**负责人：** [李宛纹]

## 1. 实验目的
通过调试 QEMU 源码，从外部视角观察 RISC-V 架构下硬件 MMU 是如何一步步完成虚拟地址到物理地址转换的，对比软件模拟与硬件逻辑的异同。

## 2. 实验环境与连接

### 2.1 关联 ucore
修改 Makefile，将 QEMU 路径指向编译好的 Debug 版本。

![alt text](image.png)

### 2.2 双重 GDB 连接
建立了三终端调试环境：终端1启动 QEMU，终端2调试 QEMU 进程，终端3调试 ucore 内核。

![alt text](image-1.png)

## 3. 调试策略调整与实施
### 3.1 问题遇阻
在尝试使用 Host GDB 调试 QEMU 源码时，由于实验环境中的 QEMU 为系统预编译版本，被剥离了调试符号表 (Stripped)，导致无法在模拟器层面拦截地址翻译函数。
为了完成实验目标，我将策略调整为 **从操作系统视角 (Guest GDB) 验证页表机制**。

### 3.2 关键验证过程

**1. 拦截内核初始化**
我在 ucore 内核启动页表机制的关键函数 `pmm_init` 处设置断点，成功拦截了内存初始化流程。

![alt text](image-2.png)

**2. 验证页表基址寄存器 (SATP)**
由于符号表限制，我直接通过 GDB 读取 RISC-V 架构的核心寄存器 **SATP** (Supervisor Address Translation and Protection)。这是 MMU 硬件进行地址翻译的“根目录”。

![alt text](image-3.png)

此时 SATP 寄存器的值为 0x8000000000080205，这证明了操作系统已经完成了页表基址的配置，MMU 硬件即将利用该地址进行虚拟地址翻译。
4. 实验心得
本次实验经历了一个“发现问题-分析问题-解决问题”的过程。 起初因为 QEMU 缺少符号表导致无法按原计划调试模拟器源码。面对这个环境限制，我意识到页表机制的验证不一定非要看“模拟器怎么查表”，看“操作系统怎么建表”也是极佳的切入点。 通过调试 ucore 的 pmm.c 相关代码，我复习了 SV39 页表项的结构，理解了操作系统在启动阶段是如何通过填充 PTE (页表项) 来告诉硬件（MMU）应该如何翻译地址的。这种灵活调整调试视角的经历，比单纯照着文档做一遍更有收获。