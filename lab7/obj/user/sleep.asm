
obj/__user_sleep.out:     file format elf64-littleriscv


Disassembly of section .text:

0000000000800020 <_start>:
    # move down the esp register
    # since it may cause page fault in backtrace
    // subl $0x20, %esp

    # call user-program function
    call umain
  800020:	132000ef          	jal	800152 <umain>
1:  j 1b
  800024:	a001                	j	800024 <_start+0x4>

0000000000800026 <__panic>:
#include <stdio.h>
#include <ulib.h>
#include <error.h>

void
__panic(const char *file, int line, const char *fmt, ...) {
  800026:	715d                	addi	sp,sp,-80
    // print the 'message'
    va_list ap;
    va_start(ap, fmt);
  800028:	02810313          	addi	t1,sp,40
__panic(const char *file, int line, const char *fmt, ...) {
  80002c:	e822                	sd	s0,16(sp)
  80002e:	8432                	mv	s0,a2
    cprintf("user panic at %s:%d:\n    ", file, line);
  800030:	862e                	mv	a2,a1
  800032:	85aa                	mv	a1,a0
  800034:	00000517          	auipc	a0,0x0
  800038:	5b450513          	addi	a0,a0,1460 # 8005e8 <main+0x70>
__panic(const char *file, int line, const char *fmt, ...) {
  80003c:	ec06                	sd	ra,24(sp)
  80003e:	f436                	sd	a3,40(sp)
  800040:	f83a                	sd	a4,48(sp)
  800042:	fc3e                	sd	a5,56(sp)
  800044:	e0c2                	sd	a6,64(sp)
  800046:	e4c6                	sd	a7,72(sp)
    va_start(ap, fmt);
  800048:	e41a                	sd	t1,8(sp)
    cprintf("user panic at %s:%d:\n    ", file, line);
  80004a:	056000ef          	jal	8000a0 <cprintf>
    vcprintf(fmt, ap);
  80004e:	65a2                	ld	a1,8(sp)
  800050:	8522                	mv	a0,s0
  800052:	02e000ef          	jal	800080 <vcprintf>
    cprintf("\n");
  800056:	00000517          	auipc	a0,0x0
  80005a:	5b250513          	addi	a0,a0,1458 # 800608 <main+0x90>
  80005e:	042000ef          	jal	8000a0 <cprintf>
    va_end(ap);
    exit(-E_PANIC);
  800062:	5559                	li	a0,-10
  800064:	0cc000ef          	jal	800130 <exit>

0000000000800068 <cputch>:
/* *
 * cputch - writes a single character @c to stdout, and it will
 * increace the value of counter pointed by @cnt.
 * */
static void
cputch(int c, int *cnt) {
  800068:	1101                	addi	sp,sp,-32
  80006a:	ec06                	sd	ra,24(sp)
  80006c:	e42e                	sd	a1,8(sp)
    sys_putc(c);
  80006e:	0b2000ef          	jal	800120 <sys_putc>
    (*cnt) ++;
  800072:	65a2                	ld	a1,8(sp)
}
  800074:	60e2                	ld	ra,24(sp)
    (*cnt) ++;
  800076:	419c                	lw	a5,0(a1)
  800078:	2785                	addiw	a5,a5,1
  80007a:	c19c                	sw	a5,0(a1)
}
  80007c:	6105                	addi	sp,sp,32
  80007e:	8082                	ret

0000000000800080 <vcprintf>:
 *
 * Call this function if you are already dealing with a va_list.
 * Or you probably want cprintf() instead.
 * */
int
vcprintf(const char *fmt, va_list ap) {
  800080:	1101                	addi	sp,sp,-32
  800082:	862a                	mv	a2,a0
  800084:	86ae                	mv	a3,a1
    int cnt = 0;
    vprintfmt((void*)cputch, &cnt, fmt, ap);
  800086:	00000517          	auipc	a0,0x0
  80008a:	fe250513          	addi	a0,a0,-30 # 800068 <cputch>
  80008e:	006c                	addi	a1,sp,12
vcprintf(const char *fmt, va_list ap) {
  800090:	ec06                	sd	ra,24(sp)
    int cnt = 0;
  800092:	c602                	sw	zero,12(sp)
    vprintfmt((void*)cputch, &cnt, fmt, ap);
  800094:	130000ef          	jal	8001c4 <vprintfmt>
    return cnt;
}
  800098:	60e2                	ld	ra,24(sp)
  80009a:	4532                	lw	a0,12(sp)
  80009c:	6105                	addi	sp,sp,32
  80009e:	8082                	ret

00000000008000a0 <cprintf>:
 *
 * The return value is the number of characters which would be
 * written to stdout.
 * */
int
cprintf(const char *fmt, ...) {
  8000a0:	711d                	addi	sp,sp,-96
    va_list ap;

    va_start(ap, fmt);
  8000a2:	02810313          	addi	t1,sp,40
cprintf(const char *fmt, ...) {
  8000a6:	f42e                	sd	a1,40(sp)
  8000a8:	f832                	sd	a2,48(sp)
  8000aa:	fc36                	sd	a3,56(sp)
    vprintfmt((void*)cputch, &cnt, fmt, ap);
  8000ac:	862a                	mv	a2,a0
  8000ae:	004c                	addi	a1,sp,4
  8000b0:	00000517          	auipc	a0,0x0
  8000b4:	fb850513          	addi	a0,a0,-72 # 800068 <cputch>
  8000b8:	869a                	mv	a3,t1
cprintf(const char *fmt, ...) {
  8000ba:	ec06                	sd	ra,24(sp)
  8000bc:	e0ba                	sd	a4,64(sp)
  8000be:	e4be                	sd	a5,72(sp)
  8000c0:	e8c2                	sd	a6,80(sp)
  8000c2:	ecc6                	sd	a7,88(sp)
    int cnt = 0;
  8000c4:	c202                	sw	zero,4(sp)
    va_start(ap, fmt);
  8000c6:	e41a                	sd	t1,8(sp)
    vprintfmt((void*)cputch, &cnt, fmt, ap);
  8000c8:	0fc000ef          	jal	8001c4 <vprintfmt>
    int cnt = vcprintf(fmt, ap);
    va_end(ap);

    return cnt;
}
  8000cc:	60e2                	ld	ra,24(sp)
  8000ce:	4512                	lw	a0,4(sp)
  8000d0:	6125                	addi	sp,sp,96
  8000d2:	8082                	ret

00000000008000d4 <syscall>:
#include <syscall.h>

#define MAX_ARGS            5

static inline int
syscall(int64_t num, ...) {
  8000d4:	7175                	addi	sp,sp,-144
    va_list ap;
    va_start(ap, num);
    uint64_t a[MAX_ARGS];
    int i, ret;
    for (i = 0; i < MAX_ARGS; i ++) {
        a[i] = va_arg(ap, uint64_t);
  8000d6:	08010313          	addi	t1,sp,128
syscall(int64_t num, ...) {
  8000da:	e42a                	sd	a0,8(sp)
  8000dc:	ecae                	sd	a1,88(sp)
        a[i] = va_arg(ap, uint64_t);
  8000de:	f42e                	sd	a1,40(sp)
syscall(int64_t num, ...) {
  8000e0:	f0b2                	sd	a2,96(sp)
        a[i] = va_arg(ap, uint64_t);
  8000e2:	f832                	sd	a2,48(sp)
syscall(int64_t num, ...) {
  8000e4:	f4b6                	sd	a3,104(sp)
        a[i] = va_arg(ap, uint64_t);
  8000e6:	fc36                	sd	a3,56(sp)
syscall(int64_t num, ...) {
  8000e8:	f8ba                	sd	a4,112(sp)
        a[i] = va_arg(ap, uint64_t);
  8000ea:	e0ba                	sd	a4,64(sp)
syscall(int64_t num, ...) {
  8000ec:	fcbe                	sd	a5,120(sp)
        a[i] = va_arg(ap, uint64_t);
  8000ee:	e4be                	sd	a5,72(sp)
syscall(int64_t num, ...) {
  8000f0:	e142                	sd	a6,128(sp)
  8000f2:	e546                	sd	a7,136(sp)
        a[i] = va_arg(ap, uint64_t);
  8000f4:	f01a                	sd	t1,32(sp)
    }
    va_end(ap);

    asm volatile (
  8000f6:	4522                	lw	a0,8(sp)
  8000f8:	55a2                	lw	a1,40(sp)
  8000fa:	5642                	lw	a2,48(sp)
  8000fc:	56e2                	lw	a3,56(sp)
  8000fe:	4706                	lw	a4,64(sp)
  800100:	47a6                	lw	a5,72(sp)
  800102:	00000073          	ecall
  800106:	ce2a                	sw	a0,28(sp)
          "m" (a[3]),
          "m" (a[4])
        : "memory"
      );
    return ret;
}
  800108:	4572                	lw	a0,28(sp)
  80010a:	6149                	addi	sp,sp,144
  80010c:	8082                	ret

000000000080010e <sys_exit>:

int
sys_exit(int64_t error_code) {
  80010e:	85aa                	mv	a1,a0
    return syscall(SYS_exit, error_code);
  800110:	4505                	li	a0,1
  800112:	b7c9                	j	8000d4 <syscall>

0000000000800114 <sys_fork>:
}

int
sys_fork(void) {
    return syscall(SYS_fork);
  800114:	4509                	li	a0,2
  800116:	bf7d                	j	8000d4 <syscall>

0000000000800118 <sys_wait>:
}

int
sys_wait(int64_t pid, int *store) {
  800118:	862e                	mv	a2,a1
    return syscall(SYS_wait, pid, store);
  80011a:	85aa                	mv	a1,a0
  80011c:	450d                	li	a0,3
  80011e:	bf5d                	j	8000d4 <syscall>

0000000000800120 <sys_putc>:
sys_getpid(void) {
    return syscall(SYS_getpid);
}

int
sys_putc(int64_t c) {
  800120:	85aa                	mv	a1,a0
    return syscall(SYS_putc, c);
  800122:	4579                	li	a0,30
  800124:	bf45                	j	8000d4 <syscall>

0000000000800126 <sys_gettime>:
    return syscall(SYS_pgdir);
}

int
sys_gettime(void) {
    return syscall(SYS_gettime);
  800126:	4545                	li	a0,17
  800128:	b775                	j	8000d4 <syscall>

000000000080012a <sys_sleep>:
{
    syscall(SYS_lab6_set_priority, priority);
}

int
sys_sleep(uint64_t time) {
  80012a:	85aa                	mv	a1,a0
    return syscall(SYS_sleep, time);
  80012c:	452d                	li	a0,11
  80012e:	b75d                	j	8000d4 <syscall>

0000000000800130 <exit>:
#include <syscall.h>
#include <stdio.h>
#include <ulib.h>

void
exit(int error_code) {
  800130:	1141                	addi	sp,sp,-16
  800132:	e406                	sd	ra,8(sp)
    sys_exit(error_code);
  800134:	fdbff0ef          	jal	80010e <sys_exit>
    cprintf("BUG: exit failed.\n");
  800138:	00000517          	auipc	a0,0x0
  80013c:	4d850513          	addi	a0,a0,1240 # 800610 <main+0x98>
  800140:	f61ff0ef          	jal	8000a0 <cprintf>
    while (1);
  800144:	a001                	j	800144 <exit+0x14>

0000000000800146 <fork>:
}

int
fork(void) {
    return sys_fork();
  800146:	b7f9                	j	800114 <sys_fork>

0000000000800148 <waitpid>:
    return sys_wait(0, NULL);
}

int
waitpid(int pid, int *store) {
    return sys_wait(pid, store);
  800148:	bfc1                	j	800118 <sys_wait>

000000000080014a <gettime_msec>:
    sys_pgdir();
}

unsigned int
gettime_msec(void) {
    return (unsigned int)sys_gettime();
  80014a:	bff1                	j	800126 <sys_gettime>

000000000080014c <sleep>:
    sys_lab6_set_priority(priority);
}

int
sleep(unsigned int time) {
    return sys_sleep(time);
  80014c:	1502                	slli	a0,a0,0x20
  80014e:	9101                	srli	a0,a0,0x20
  800150:	bfe9                	j	80012a <sys_sleep>

0000000000800152 <umain>:
#include <ulib.h>

int main(void);

void
umain(void) {
  800152:	1141                	addi	sp,sp,-16
  800154:	e406                	sd	ra,8(sp)
    int ret = main();
  800156:	422000ef          	jal	800578 <main>
    exit(ret);
  80015a:	fd7ff0ef          	jal	800130 <exit>

000000000080015e <printnum>:
 * @width:      maximum number of digits, if the actual width is less than @width, use @padc instead
 * @padc:       character that padded on the left if the actual width is less than @width
 * */
static void
printnum(void (*putch)(int, void*), void *putdat,
        unsigned long long num, unsigned base, int width, int padc) {
  80015e:	7179                	addi	sp,sp,-48
    unsigned long long result = num;
    unsigned mod = do_div(result, base);
  800160:	02069813          	slli	a6,a3,0x20
        unsigned long long num, unsigned base, int width, int padc) {
  800164:	f022                	sd	s0,32(sp)
  800166:	ec26                	sd	s1,24(sp)
  800168:	e84a                	sd	s2,16(sp)
  80016a:	e052                	sd	s4,0(sp)
    unsigned mod = do_div(result, base);
  80016c:	02085813          	srli	a6,a6,0x20
        unsigned long long num, unsigned base, int width, int padc) {
  800170:	f406                	sd	ra,40(sp)
    unsigned mod = do_div(result, base);
  800172:	03067a33          	remu	s4,a2,a6
    // first recursively print all preceding (more significant) digits
    if (num >= base) {
        printnum(putch, putdat, result, base, width - 1, padc);
    } else {
        // print any needed pad characters before first digit
        while (-- width > 0)
  800176:	fff7041b          	addiw	s0,a4,-1
        unsigned long long num, unsigned base, int width, int padc) {
  80017a:	84aa                	mv	s1,a0
  80017c:	892e                	mv	s2,a1
    if (num >= base) {
  80017e:	03067d63          	bgeu	a2,a6,8001b8 <printnum+0x5a>
  800182:	e44e                	sd	s3,8(sp)
  800184:	89be                	mv	s3,a5
        while (-- width > 0)
  800186:	4785                	li	a5,1
  800188:	00e7d763          	bge	a5,a4,800196 <printnum+0x38>
            putch(padc, putdat);
  80018c:	85ca                	mv	a1,s2
  80018e:	854e                	mv	a0,s3
        while (-- width > 0)
  800190:	347d                	addiw	s0,s0,-1
            putch(padc, putdat);
  800192:	9482                	jalr	s1
        while (-- width > 0)
  800194:	fc65                	bnez	s0,80018c <printnum+0x2e>
  800196:	69a2                	ld	s3,8(sp)
    }
    // then print this (the least significant) digit
    putch("0123456789abcdef"[mod], putdat);
  800198:	00000797          	auipc	a5,0x0
  80019c:	49078793          	addi	a5,a5,1168 # 800628 <main+0xb0>
  8001a0:	97d2                	add	a5,a5,s4
    // Crashes if num >= base. No idea what going on here
    // Here is a quick fix
    // update: Stack grows downward and destory the SBI
    // sbi_console_putchar("0123456789abcdef"[mod]);
    // (*(int *)putdat)++;
}
  8001a2:	7402                	ld	s0,32(sp)
    putch("0123456789abcdef"[mod], putdat);
  8001a4:	0007c503          	lbu	a0,0(a5)
}
  8001a8:	70a2                	ld	ra,40(sp)
  8001aa:	6a02                	ld	s4,0(sp)
    putch("0123456789abcdef"[mod], putdat);
  8001ac:	85ca                	mv	a1,s2
  8001ae:	87a6                	mv	a5,s1
}
  8001b0:	6942                	ld	s2,16(sp)
  8001b2:	64e2                	ld	s1,24(sp)
  8001b4:	6145                	addi	sp,sp,48
    putch("0123456789abcdef"[mod], putdat);
  8001b6:	8782                	jr	a5
        printnum(putch, putdat, result, base, width - 1, padc);
  8001b8:	03065633          	divu	a2,a2,a6
  8001bc:	8722                	mv	a4,s0
  8001be:	fa1ff0ef          	jal	80015e <printnum>
  8001c2:	bfd9                	j	800198 <printnum+0x3a>

00000000008001c4 <vprintfmt>:
 *
 * Call this function if you are already dealing with a va_list.
 * Or you probably want printfmt() instead.
 * */
void
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap) {
  8001c4:	7119                	addi	sp,sp,-128
  8001c6:	f4a6                	sd	s1,104(sp)
  8001c8:	f0ca                	sd	s2,96(sp)
  8001ca:	ecce                	sd	s3,88(sp)
  8001cc:	e8d2                	sd	s4,80(sp)
  8001ce:	e4d6                	sd	s5,72(sp)
  8001d0:	e0da                	sd	s6,64(sp)
  8001d2:	f862                	sd	s8,48(sp)
  8001d4:	fc86                	sd	ra,120(sp)
  8001d6:	f8a2                	sd	s0,112(sp)
  8001d8:	fc5e                	sd	s7,56(sp)
  8001da:	f466                	sd	s9,40(sp)
  8001dc:	f06a                	sd	s10,32(sp)
  8001de:	ec6e                	sd	s11,24(sp)
  8001e0:	84aa                	mv	s1,a0
  8001e2:	8c32                	mv	s8,a2
  8001e4:	8a36                	mv	s4,a3
  8001e6:	892e                	mv	s2,a1
    register int ch, err;
    unsigned long long num;
    int base, width, precision, lflag, altflag;

    while (1) {
        while ((ch = *(unsigned char *)fmt ++) != '%') {
  8001e8:	02500993          	li	s3,37
        char padc = ' ';
        width = precision = -1;
        lflag = altflag = 0;

    reswitch:
        switch (ch = *(unsigned char *)fmt ++) {
  8001ec:	05500b13          	li	s6,85
  8001f0:	00000a97          	auipc	s5,0x0
  8001f4:	5d0a8a93          	addi	s5,s5,1488 # 8007c0 <main+0x248>
        while ((ch = *(unsigned char *)fmt ++) != '%') {
  8001f8:	000c4503          	lbu	a0,0(s8)
  8001fc:	001c0413          	addi	s0,s8,1
  800200:	01350a63          	beq	a0,s3,800214 <vprintfmt+0x50>
            if (ch == '\0') {
  800204:	cd0d                	beqz	a0,80023e <vprintfmt+0x7a>
            putch(ch, putdat);
  800206:	85ca                	mv	a1,s2
  800208:	9482                	jalr	s1
        while ((ch = *(unsigned char *)fmt ++) != '%') {
  80020a:	00044503          	lbu	a0,0(s0)
  80020e:	0405                	addi	s0,s0,1
  800210:	ff351ae3          	bne	a0,s3,800204 <vprintfmt+0x40>
        width = precision = -1;
  800214:	5cfd                	li	s9,-1
  800216:	8d66                	mv	s10,s9
        char padc = ' ';
  800218:	02000d93          	li	s11,32
        lflag = altflag = 0;
  80021c:	4b81                	li	s7,0
  80021e:	4781                	li	a5,0
        switch (ch = *(unsigned char *)fmt ++) {
  800220:	00044683          	lbu	a3,0(s0)
  800224:	00140c13          	addi	s8,s0,1
  800228:	fdd6859b          	addiw	a1,a3,-35
  80022c:	0ff5f593          	zext.b	a1,a1
  800230:	02bb6663          	bltu	s6,a1,80025c <vprintfmt+0x98>
  800234:	058a                	slli	a1,a1,0x2
  800236:	95d6                	add	a1,a1,s5
  800238:	4198                	lw	a4,0(a1)
  80023a:	9756                	add	a4,a4,s5
  80023c:	8702                	jr	a4
            for (fmt --; fmt[-1] != '%'; fmt --)
                /* do nothing */;
            break;
        }
    }
}
  80023e:	70e6                	ld	ra,120(sp)
  800240:	7446                	ld	s0,112(sp)
  800242:	74a6                	ld	s1,104(sp)
  800244:	7906                	ld	s2,96(sp)
  800246:	69e6                	ld	s3,88(sp)
  800248:	6a46                	ld	s4,80(sp)
  80024a:	6aa6                	ld	s5,72(sp)
  80024c:	6b06                	ld	s6,64(sp)
  80024e:	7be2                	ld	s7,56(sp)
  800250:	7c42                	ld	s8,48(sp)
  800252:	7ca2                	ld	s9,40(sp)
  800254:	7d02                	ld	s10,32(sp)
  800256:	6de2                	ld	s11,24(sp)
  800258:	6109                	addi	sp,sp,128
  80025a:	8082                	ret
            putch('%', putdat);
  80025c:	85ca                	mv	a1,s2
  80025e:	02500513          	li	a0,37
  800262:	9482                	jalr	s1
            for (fmt --; fmt[-1] != '%'; fmt --)
  800264:	fff44783          	lbu	a5,-1(s0)
  800268:	02500713          	li	a4,37
  80026c:	8c22                	mv	s8,s0
  80026e:	f8e785e3          	beq	a5,a4,8001f8 <vprintfmt+0x34>
  800272:	ffec4783          	lbu	a5,-2(s8)
  800276:	1c7d                	addi	s8,s8,-1
  800278:	fee79de3          	bne	a5,a4,800272 <vprintfmt+0xae>
  80027c:	bfb5                	j	8001f8 <vprintfmt+0x34>
                ch = *fmt;
  80027e:	00144603          	lbu	a2,1(s0)
                if (ch < '0' || ch > '9') {
  800282:	4525                	li	a0,9
                precision = precision * 10 + ch - '0';
  800284:	fd068c9b          	addiw	s9,a3,-48
                if (ch < '0' || ch > '9') {
  800288:	fd06071b          	addiw	a4,a2,-48
  80028c:	24e56a63          	bltu	a0,a4,8004e0 <vprintfmt+0x31c>
                ch = *fmt;
  800290:	2601                	sext.w	a2,a2
        switch (ch = *(unsigned char *)fmt ++) {
  800292:	8462                	mv	s0,s8
                precision = precision * 10 + ch - '0';
  800294:	002c971b          	slliw	a4,s9,0x2
                ch = *fmt;
  800298:	00144683          	lbu	a3,1(s0)
                precision = precision * 10 + ch - '0';
  80029c:	0197073b          	addw	a4,a4,s9
  8002a0:	0017171b          	slliw	a4,a4,0x1
  8002a4:	9f31                	addw	a4,a4,a2
                if (ch < '0' || ch > '9') {
  8002a6:	fd06859b          	addiw	a1,a3,-48
            for (precision = 0; ; ++ fmt) {
  8002aa:	0405                	addi	s0,s0,1
                precision = precision * 10 + ch - '0';
  8002ac:	fd070c9b          	addiw	s9,a4,-48
                ch = *fmt;
  8002b0:	0006861b          	sext.w	a2,a3
                if (ch < '0' || ch > '9') {
  8002b4:	feb570e3          	bgeu	a0,a1,800294 <vprintfmt+0xd0>
            if (width < 0)
  8002b8:	f60d54e3          	bgez	s10,800220 <vprintfmt+0x5c>
                width = precision, precision = -1;
  8002bc:	8d66                	mv	s10,s9
  8002be:	5cfd                	li	s9,-1
  8002c0:	b785                	j	800220 <vprintfmt+0x5c>
        switch (ch = *(unsigned char *)fmt ++) {
  8002c2:	8db6                	mv	s11,a3
  8002c4:	8462                	mv	s0,s8
  8002c6:	bfa9                	j	800220 <vprintfmt+0x5c>
  8002c8:	8462                	mv	s0,s8
            altflag = 1;
  8002ca:	4b85                	li	s7,1
            goto reswitch;
  8002cc:	bf91                	j	800220 <vprintfmt+0x5c>
    if (lflag >= 2) {
  8002ce:	4705                	li	a4,1
            precision = va_arg(ap, int);
  8002d0:	008a0593          	addi	a1,s4,8
    if (lflag >= 2) {
  8002d4:	00f74463          	blt	a4,a5,8002dc <vprintfmt+0x118>
    else if (lflag) {
  8002d8:	1a078763          	beqz	a5,800486 <vprintfmt+0x2c2>
        return va_arg(*ap, unsigned long);
  8002dc:	000a3603          	ld	a2,0(s4)
  8002e0:	46c1                	li	a3,16
  8002e2:	8a2e                	mv	s4,a1
            printnum(putch, putdat, num, base, width, padc);
  8002e4:	000d879b          	sext.w	a5,s11
  8002e8:	876a                	mv	a4,s10
  8002ea:	85ca                	mv	a1,s2
  8002ec:	8526                	mv	a0,s1
  8002ee:	e71ff0ef          	jal	80015e <printnum>
            break;
  8002f2:	b719                	j	8001f8 <vprintfmt+0x34>
            putch(va_arg(ap, int), putdat);
  8002f4:	000a2503          	lw	a0,0(s4)
  8002f8:	85ca                	mv	a1,s2
  8002fa:	0a21                	addi	s4,s4,8
  8002fc:	9482                	jalr	s1
            break;
  8002fe:	bded                	j	8001f8 <vprintfmt+0x34>
    if (lflag >= 2) {
  800300:	4705                	li	a4,1
            precision = va_arg(ap, int);
  800302:	008a0593          	addi	a1,s4,8
    if (lflag >= 2) {
  800306:	00f74463          	blt	a4,a5,80030e <vprintfmt+0x14a>
    else if (lflag) {
  80030a:	16078963          	beqz	a5,80047c <vprintfmt+0x2b8>
        return va_arg(*ap, unsigned long);
  80030e:	000a3603          	ld	a2,0(s4)
  800312:	46a9                	li	a3,10
  800314:	8a2e                	mv	s4,a1
  800316:	b7f9                	j	8002e4 <vprintfmt+0x120>
            putch('0', putdat);
  800318:	85ca                	mv	a1,s2
  80031a:	03000513          	li	a0,48
  80031e:	9482                	jalr	s1
            putch('x', putdat);
  800320:	85ca                	mv	a1,s2
  800322:	07800513          	li	a0,120
  800326:	9482                	jalr	s1
            num = (unsigned long long)(uintptr_t)va_arg(ap, void *);
  800328:	000a3603          	ld	a2,0(s4)
            goto number;
  80032c:	46c1                	li	a3,16
            num = (unsigned long long)(uintptr_t)va_arg(ap, void *);
  80032e:	0a21                	addi	s4,s4,8
            goto number;
  800330:	bf55                	j	8002e4 <vprintfmt+0x120>
            putch(ch, putdat);
  800332:	85ca                	mv	a1,s2
  800334:	02500513          	li	a0,37
  800338:	9482                	jalr	s1
            break;
  80033a:	bd7d                	j	8001f8 <vprintfmt+0x34>
            precision = va_arg(ap, int);
  80033c:	000a2c83          	lw	s9,0(s4)
        switch (ch = *(unsigned char *)fmt ++) {
  800340:	8462                	mv	s0,s8
            precision = va_arg(ap, int);
  800342:	0a21                	addi	s4,s4,8
            goto process_precision;
  800344:	bf95                	j	8002b8 <vprintfmt+0xf4>
    if (lflag >= 2) {
  800346:	4705                	li	a4,1
            precision = va_arg(ap, int);
  800348:	008a0593          	addi	a1,s4,8
    if (lflag >= 2) {
  80034c:	00f74463          	blt	a4,a5,800354 <vprintfmt+0x190>
    else if (lflag) {
  800350:	12078163          	beqz	a5,800472 <vprintfmt+0x2ae>
        return va_arg(*ap, unsigned long);
  800354:	000a3603          	ld	a2,0(s4)
  800358:	46a1                	li	a3,8
  80035a:	8a2e                	mv	s4,a1
  80035c:	b761                	j	8002e4 <vprintfmt+0x120>
            if (width < 0)
  80035e:	876a                	mv	a4,s10
  800360:	000d5363          	bgez	s10,800366 <vprintfmt+0x1a2>
  800364:	4701                	li	a4,0
  800366:	00070d1b          	sext.w	s10,a4
        switch (ch = *(unsigned char *)fmt ++) {
  80036a:	8462                	mv	s0,s8
            goto reswitch;
  80036c:	bd55                	j	800220 <vprintfmt+0x5c>
            if (width > 0 && padc != '-') {
  80036e:	000d841b          	sext.w	s0,s11
  800372:	fd340793          	addi	a5,s0,-45
  800376:	00f037b3          	snez	a5,a5
  80037a:	01a02733          	sgtz	a4,s10
            if ((p = va_arg(ap, char *)) == NULL) {
  80037e:	000a3d83          	ld	s11,0(s4)
            if (width > 0 && padc != '-') {
  800382:	8f7d                	and	a4,a4,a5
            if ((p = va_arg(ap, char *)) == NULL) {
  800384:	008a0793          	addi	a5,s4,8
  800388:	e43e                	sd	a5,8(sp)
  80038a:	100d8c63          	beqz	s11,8004a2 <vprintfmt+0x2de>
            if (width > 0 && padc != '-') {
  80038e:	12071363          	bnez	a4,8004b4 <vprintfmt+0x2f0>
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
  800392:	000dc783          	lbu	a5,0(s11)
  800396:	0007851b          	sext.w	a0,a5
  80039a:	c78d                	beqz	a5,8003c4 <vprintfmt+0x200>
  80039c:	0d85                	addi	s11,s11,1
  80039e:	547d                	li	s0,-1
                if (altflag && (ch < ' ' || ch > '~')) {
  8003a0:	05e00a13          	li	s4,94
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
  8003a4:	000cc563          	bltz	s9,8003ae <vprintfmt+0x1ea>
  8003a8:	3cfd                	addiw	s9,s9,-1
  8003aa:	008c8d63          	beq	s9,s0,8003c4 <vprintfmt+0x200>
                if (altflag && (ch < ' ' || ch > '~')) {
  8003ae:	020b9663          	bnez	s7,8003da <vprintfmt+0x216>
                    putch(ch, putdat);
  8003b2:	85ca                	mv	a1,s2
  8003b4:	9482                	jalr	s1
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
  8003b6:	000dc783          	lbu	a5,0(s11)
  8003ba:	0d85                	addi	s11,s11,1
  8003bc:	3d7d                	addiw	s10,s10,-1
  8003be:	0007851b          	sext.w	a0,a5
  8003c2:	f3ed                	bnez	a5,8003a4 <vprintfmt+0x1e0>
            for (; width > 0; width --) {
  8003c4:	01a05963          	blez	s10,8003d6 <vprintfmt+0x212>
                putch(' ', putdat);
  8003c8:	85ca                	mv	a1,s2
  8003ca:	02000513          	li	a0,32
            for (; width > 0; width --) {
  8003ce:	3d7d                	addiw	s10,s10,-1
                putch(' ', putdat);
  8003d0:	9482                	jalr	s1
            for (; width > 0; width --) {
  8003d2:	fe0d1be3          	bnez	s10,8003c8 <vprintfmt+0x204>
            if ((p = va_arg(ap, char *)) == NULL) {
  8003d6:	6a22                	ld	s4,8(sp)
  8003d8:	b505                	j	8001f8 <vprintfmt+0x34>
                if (altflag && (ch < ' ' || ch > '~')) {
  8003da:	3781                	addiw	a5,a5,-32
  8003dc:	fcfa7be3          	bgeu	s4,a5,8003b2 <vprintfmt+0x1ee>
                    putch('?', putdat);
  8003e0:	03f00513          	li	a0,63
  8003e4:	85ca                	mv	a1,s2
  8003e6:	9482                	jalr	s1
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
  8003e8:	000dc783          	lbu	a5,0(s11)
  8003ec:	0d85                	addi	s11,s11,1
  8003ee:	3d7d                	addiw	s10,s10,-1
  8003f0:	0007851b          	sext.w	a0,a5
  8003f4:	dbe1                	beqz	a5,8003c4 <vprintfmt+0x200>
  8003f6:	fa0cd9e3          	bgez	s9,8003a8 <vprintfmt+0x1e4>
  8003fa:	b7c5                	j	8003da <vprintfmt+0x216>
            if (err < 0) {
  8003fc:	000a2783          	lw	a5,0(s4)
            if (err > MAXERROR || (p = error_string[err]) == NULL) {
  800400:	4661                	li	a2,24
            err = va_arg(ap, int);
  800402:	0a21                	addi	s4,s4,8
            if (err < 0) {
  800404:	41f7d71b          	sraiw	a4,a5,0x1f
  800408:	8fb9                	xor	a5,a5,a4
  80040a:	40e786bb          	subw	a3,a5,a4
            if (err > MAXERROR || (p = error_string[err]) == NULL) {
  80040e:	02d64563          	blt	a2,a3,800438 <vprintfmt+0x274>
  800412:	00000797          	auipc	a5,0x0
  800416:	50678793          	addi	a5,a5,1286 # 800918 <error_string>
  80041a:	00369713          	slli	a4,a3,0x3
  80041e:	97ba                	add	a5,a5,a4
  800420:	639c                	ld	a5,0(a5)
  800422:	cb99                	beqz	a5,800438 <vprintfmt+0x274>
                printfmt(putch, putdat, "%s", p);
  800424:	86be                	mv	a3,a5
  800426:	00000617          	auipc	a2,0x0
  80042a:	23260613          	addi	a2,a2,562 # 800658 <main+0xe0>
  80042e:	85ca                	mv	a1,s2
  800430:	8526                	mv	a0,s1
  800432:	0d8000ef          	jal	80050a <printfmt>
  800436:	b3c9                	j	8001f8 <vprintfmt+0x34>
                printfmt(putch, putdat, "error %d", err);
  800438:	00000617          	auipc	a2,0x0
  80043c:	21060613          	addi	a2,a2,528 # 800648 <main+0xd0>
  800440:	85ca                	mv	a1,s2
  800442:	8526                	mv	a0,s1
  800444:	0c6000ef          	jal	80050a <printfmt>
  800448:	bb45                	j	8001f8 <vprintfmt+0x34>
    if (lflag >= 2) {
  80044a:	4705                	li	a4,1
            precision = va_arg(ap, int);
  80044c:	008a0b93          	addi	s7,s4,8
    if (lflag >= 2) {
  800450:	00f74363          	blt	a4,a5,800456 <vprintfmt+0x292>
    else if (lflag) {
  800454:	cf81                	beqz	a5,80046c <vprintfmt+0x2a8>
        return va_arg(*ap, long);
  800456:	000a3403          	ld	s0,0(s4)
            if ((long long)num < 0) {
  80045a:	02044b63          	bltz	s0,800490 <vprintfmt+0x2cc>
            num = getint(&ap, lflag);
  80045e:	8622                	mv	a2,s0
  800460:	8a5e                	mv	s4,s7
  800462:	46a9                	li	a3,10
  800464:	b541                	j	8002e4 <vprintfmt+0x120>
            lflag ++;
  800466:	2785                	addiw	a5,a5,1
        switch (ch = *(unsigned char *)fmt ++) {
  800468:	8462                	mv	s0,s8
            goto reswitch;
  80046a:	bb5d                	j	800220 <vprintfmt+0x5c>
        return va_arg(*ap, int);
  80046c:	000a2403          	lw	s0,0(s4)
  800470:	b7ed                	j	80045a <vprintfmt+0x296>
        return va_arg(*ap, unsigned int);
  800472:	000a6603          	lwu	a2,0(s4)
  800476:	46a1                	li	a3,8
  800478:	8a2e                	mv	s4,a1
  80047a:	b5ad                	j	8002e4 <vprintfmt+0x120>
  80047c:	000a6603          	lwu	a2,0(s4)
  800480:	46a9                	li	a3,10
  800482:	8a2e                	mv	s4,a1
  800484:	b585                	j	8002e4 <vprintfmt+0x120>
  800486:	000a6603          	lwu	a2,0(s4)
  80048a:	46c1                	li	a3,16
  80048c:	8a2e                	mv	s4,a1
  80048e:	bd99                	j	8002e4 <vprintfmt+0x120>
                putch('-', putdat);
  800490:	85ca                	mv	a1,s2
  800492:	02d00513          	li	a0,45
  800496:	9482                	jalr	s1
                num = -(long long)num;
  800498:	40800633          	neg	a2,s0
  80049c:	8a5e                	mv	s4,s7
  80049e:	46a9                	li	a3,10
  8004a0:	b591                	j	8002e4 <vprintfmt+0x120>
            if (width > 0 && padc != '-') {
  8004a2:	e329                	bnez	a4,8004e4 <vprintfmt+0x320>
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
  8004a4:	02800793          	li	a5,40
  8004a8:	853e                	mv	a0,a5
  8004aa:	00000d97          	auipc	s11,0x0
  8004ae:	197d8d93          	addi	s11,s11,407 # 800641 <main+0xc9>
  8004b2:	b5f5                	j	80039e <vprintfmt+0x1da>
                for (width -= strnlen(p, precision); width > 0; width --) {
  8004b4:	85e6                	mv	a1,s9
  8004b6:	856e                	mv	a0,s11
  8004b8:	072000ef          	jal	80052a <strnlen>
  8004bc:	40ad0d3b          	subw	s10,s10,a0
  8004c0:	01a05863          	blez	s10,8004d0 <vprintfmt+0x30c>
                    putch(padc, putdat);
  8004c4:	85ca                	mv	a1,s2
  8004c6:	8522                	mv	a0,s0
                for (width -= strnlen(p, precision); width > 0; width --) {
  8004c8:	3d7d                	addiw	s10,s10,-1
                    putch(padc, putdat);
  8004ca:	9482                	jalr	s1
                for (width -= strnlen(p, precision); width > 0; width --) {
  8004cc:	fe0d1ce3          	bnez	s10,8004c4 <vprintfmt+0x300>
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
  8004d0:	000dc783          	lbu	a5,0(s11)
  8004d4:	0007851b          	sext.w	a0,a5
  8004d8:	ec0792e3          	bnez	a5,80039c <vprintfmt+0x1d8>
            if ((p = va_arg(ap, char *)) == NULL) {
  8004dc:	6a22                	ld	s4,8(sp)
  8004de:	bb29                	j	8001f8 <vprintfmt+0x34>
        switch (ch = *(unsigned char *)fmt ++) {
  8004e0:	8462                	mv	s0,s8
  8004e2:	bbd9                	j	8002b8 <vprintfmt+0xf4>
                for (width -= strnlen(p, precision); width > 0; width --) {
  8004e4:	85e6                	mv	a1,s9
  8004e6:	00000517          	auipc	a0,0x0
  8004ea:	15a50513          	addi	a0,a0,346 # 800640 <main+0xc8>
  8004ee:	03c000ef          	jal	80052a <strnlen>
  8004f2:	40ad0d3b          	subw	s10,s10,a0
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
  8004f6:	02800793          	li	a5,40
                p = "(null)";
  8004fa:	00000d97          	auipc	s11,0x0
  8004fe:	146d8d93          	addi	s11,s11,326 # 800640 <main+0xc8>
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
  800502:	853e                	mv	a0,a5
                for (width -= strnlen(p, precision); width > 0; width --) {
  800504:	fda040e3          	bgtz	s10,8004c4 <vprintfmt+0x300>
  800508:	bd51                	j	80039c <vprintfmt+0x1d8>

000000000080050a <printfmt>:
printfmt(void (*putch)(int, void*), void *putdat, const char *fmt, ...) {
  80050a:	715d                	addi	sp,sp,-80
    va_start(ap, fmt);
  80050c:	02810313          	addi	t1,sp,40
printfmt(void (*putch)(int, void*), void *putdat, const char *fmt, ...) {
  800510:	f436                	sd	a3,40(sp)
    vprintfmt(putch, putdat, fmt, ap);
  800512:	869a                	mv	a3,t1
printfmt(void (*putch)(int, void*), void *putdat, const char *fmt, ...) {
  800514:	ec06                	sd	ra,24(sp)
  800516:	f83a                	sd	a4,48(sp)
  800518:	fc3e                	sd	a5,56(sp)
  80051a:	e0c2                	sd	a6,64(sp)
  80051c:	e4c6                	sd	a7,72(sp)
    va_start(ap, fmt);
  80051e:	e41a                	sd	t1,8(sp)
    vprintfmt(putch, putdat, fmt, ap);
  800520:	ca5ff0ef          	jal	8001c4 <vprintfmt>
}
  800524:	60e2                	ld	ra,24(sp)
  800526:	6161                	addi	sp,sp,80
  800528:	8082                	ret

000000000080052a <strnlen>:
 * @len if there is no '\0' character among the first @len characters
 * pointed by @s.
 * */
size_t
strnlen(const char *s, size_t len) {
    size_t cnt = 0;
  80052a:	4781                	li	a5,0
    while (cnt < len && *s ++ != '\0') {
  80052c:	e589                	bnez	a1,800536 <strnlen+0xc>
  80052e:	a811                	j	800542 <strnlen+0x18>
        cnt ++;
  800530:	0785                	addi	a5,a5,1
    while (cnt < len && *s ++ != '\0') {
  800532:	00f58863          	beq	a1,a5,800542 <strnlen+0x18>
  800536:	00f50733          	add	a4,a0,a5
  80053a:	00074703          	lbu	a4,0(a4)
  80053e:	fb6d                	bnez	a4,800530 <strnlen+0x6>
  800540:	85be                	mv	a1,a5
    }
    return cnt;
}
  800542:	852e                	mv	a0,a1
  800544:	8082                	ret

0000000000800546 <sleepy>:
#include <stdio.h>
#include <ulib.h>

void
sleepy(int pid) {
  800546:	1101                	addi	sp,sp,-32
  800548:	e822                	sd	s0,16(sp)
  80054a:	e426                	sd	s1,8(sp)
  80054c:	ec06                	sd	ra,24(sp)
    int i, time = 100;
    for (i = 0; i < 10; i ++) {
  80054e:	4401                	li	s0,0
  800550:	44a9                	li	s1,10
        sleep(time);
  800552:	06400513          	li	a0,100
  800556:	bf7ff0ef          	jal	80014c <sleep>
        cprintf("sleep %d x %d slices.\n", i + 1, time);
  80055a:	2405                	addiw	s0,s0,1
  80055c:	85a2                	mv	a1,s0
  80055e:	06400613          	li	a2,100
  800562:	00000517          	auipc	a0,0x0
  800566:	1be50513          	addi	a0,a0,446 # 800720 <main+0x1a8>
  80056a:	b37ff0ef          	jal	8000a0 <cprintf>
    for (i = 0; i < 10; i ++) {
  80056e:	fe9412e3          	bne	s0,s1,800552 <sleepy+0xc>
    }
    exit(0);
  800572:	4501                	li	a0,0
  800574:	bbdff0ef          	jal	800130 <exit>

0000000000800578 <main>:
}

int
main(void) {
  800578:	1101                	addi	sp,sp,-32
  80057a:	e822                	sd	s0,16(sp)
  80057c:	ec06                	sd	ra,24(sp)
    unsigned int time = gettime_msec();
  80057e:	bcdff0ef          	jal	80014a <gettime_msec>
  800582:	842a                	mv	s0,a0
    int pid1, exit_code;

    if ((pid1 = fork()) == 0) {
  800584:	bc3ff0ef          	jal	800146 <fork>
  800588:	cd21                	beqz	a0,8005e0 <main+0x68>
        sleepy(pid1);
    }
    
    assert(waitpid(pid1, &exit_code) == 0 && exit_code == 0);
  80058a:	006c                	addi	a1,sp,12
  80058c:	bbdff0ef          	jal	800148 <waitpid>
  800590:	47b2                	lw	a5,12(sp)
  800592:	8d5d                	or	a0,a0,a5
  800594:	2501                	sext.w	a0,a0
  800596:	e515                	bnez	a0,8005c2 <main+0x4a>
    cprintf("use %04d msecs.\n", gettime_msec() - time);
  800598:	bb3ff0ef          	jal	80014a <gettime_msec>
  80059c:	408505bb          	subw	a1,a0,s0
  8005a0:	00000517          	auipc	a0,0x0
  8005a4:	1f850513          	addi	a0,a0,504 # 800798 <main+0x220>
  8005a8:	af9ff0ef          	jal	8000a0 <cprintf>
    cprintf("sleep pass.\n");
  8005ac:	00000517          	auipc	a0,0x0
  8005b0:	20450513          	addi	a0,a0,516 # 8007b0 <main+0x238>
  8005b4:	aedff0ef          	jal	8000a0 <cprintf>
    return 0;
}
  8005b8:	60e2                	ld	ra,24(sp)
  8005ba:	6442                	ld	s0,16(sp)
  8005bc:	4501                	li	a0,0
  8005be:	6105                	addi	sp,sp,32
  8005c0:	8082                	ret
    assert(waitpid(pid1, &exit_code) == 0 && exit_code == 0);
  8005c2:	00000697          	auipc	a3,0x0
  8005c6:	17668693          	addi	a3,a3,374 # 800738 <main+0x1c0>
  8005ca:	00000617          	auipc	a2,0x0
  8005ce:	1a660613          	addi	a2,a2,422 # 800770 <main+0x1f8>
  8005d2:	45dd                	li	a1,23
  8005d4:	00000517          	auipc	a0,0x0
  8005d8:	1b450513          	addi	a0,a0,436 # 800788 <main+0x210>
  8005dc:	a4bff0ef          	jal	800026 <__panic>
        sleepy(pid1);
  8005e0:	f67ff0ef          	jal	800546 <sleepy>
