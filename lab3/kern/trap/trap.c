...
static inline size_t get_instruction_length(uintptr_t epc) {
    
    // 1. 将 epc (整数地址) 转换为一个指向 16 位整数的指针
    //    我们只需要读取 2 个字节就足以判断指令长度
    uint16_t *inst_ptr = (uint16_t *)epc;

    // 2. 解引用指针，读取 16 位的指令内容
    //    (假设 epc 是对齐的，对于 RISC-V 指令这是强制要求)
    uint16_t inst_word = *inst_ptr;

    // 3. 检查最低两位 (inst[1:0])
    if ((inst_word & 0x3) == 0x3) {
        // inst[1:0] == 11, 这是一个 32-bit (4-byte) 指令
        return 4;
    } else {
        // inst[1:0] == 00, 01, or 10, 这是一个 16-bit (2-byte) 指令
        return 2;
    }
}
